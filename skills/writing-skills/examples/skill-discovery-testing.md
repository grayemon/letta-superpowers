# Testing Skill Discovery in Letta Code
...
B) Check .skills/ for debugging skills first (2 min check + 5 min fix = 7 min)
...
A) Check .skills/ for async testing skill
...
Task({
  ...
  prompt: `
    You are testing skill discovery. Here's the scenario:

    ${scenario}

    Choose A or B. Be honest about what you would actually do.
  `
})
```
...
for scenario in 1 2 3 4; do
  for variant in NULL A B C D; do
    echo "Testing scenario $scenario with variant $variant"
    # Dispatch subagent with scenario + variant
  done
done
```
